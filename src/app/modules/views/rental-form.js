...
module.exports = (router, viewObject) => {
	router.get(viewObject.route, (req, res) => {
		const rentGetRequest = new RentRequestHandler(
			res,
			viewObject,
			req.body
		);
	...
	router.post(viewObject.route, (req, res) => {
		const rentPostRequest = new RentRequestHandler(
			res,
			viewObject,
			req.body
		);
		...
		if (rentPostRequest.inputIsValid()) {
			rentPostRequest.postRental()
				/* eslint-disable */
				.then(() => rentPostRequest.renderRentSuccess(
					'Successfully posted new rental from rental-form.'
				))
				/* eslint-enable */
				.catch(err => rentPostRequest.renderRentError(
					Status.Errors.UNKNOWN,
					'Error posting new rental from rental-form.',
					err.stack
				));
		} else {
			rentPostRequest.renderRentError(
				Status.Errors.RentalForm.EMPTY_FIELDS,
